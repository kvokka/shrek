...
RSpec.describe Layers::Selector do
  ...
  it 'should lookup categories' do
    result = subject.call(json.deep_dup, categorie_ids: 11).first[:json].first['sub_themes'].first['categories'].first
    expect(result).to be_a Hash
    expect(result['id']).to eq 11
  end

  it 'should lookup indicators' do
    result = subject.call(json.deep_dup, indicator_ids: [1, 31, 32]).first[:json]
